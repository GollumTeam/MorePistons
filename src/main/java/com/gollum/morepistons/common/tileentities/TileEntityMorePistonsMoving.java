...
public class TileEntityMorePistonsMoving extends TileEntity {
	...
	public BlockMorePistonsBase pistonOrigin() {
		...
		if (b instanceof BlockMorePistonsMoving) {
			TileEntity te = this.worldObj.getBlockTileEntity(this.positionPiston.x, this.positionPiston.y, this.positionPiston.z);
				...
				if (this.storedBlock == null) {
					...
					if (this.root) {
						...
						if (this.positionPiston.equals(new Integer3d(this.xCoord, this.yCoord, this.zCoord))) {
							...
							this.worldObj.setBlock(this.xCoord, this.yCoord, this.zCoord, piston.getBlockExtention().blockID, this.storedOrientation | (piston.isSticky() ? 0x8 : 0x0), 3);
