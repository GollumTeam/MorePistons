...
public class BlockMorePistonsGravitational extends BlockMorePistonsBase {
	...
	protected void extend(World world, int x, int y, int z, int orientation, int currentOpened, int lenghtOpened)  {
		...
		Block block = Block.blocksList[world.getBlockId(xSand, ySand, zSand)];
		double i = 0;
		while (orientation == 1 && block instanceof BlockSand) {
			
			world.setBlockToAir(xSand, ySand, zSand);
			if (!world.isRemote) {
				EntityFallingSand entityFallingBlock = new EntityFallingSand(world, x2 + 0.5F, y2 + 0.5F, z2 + 0.5F, block.blockID);
			...
			block = Block.blocksList[world.getBlockId(xSand, ySand, zSand)];
			i++;
		}
