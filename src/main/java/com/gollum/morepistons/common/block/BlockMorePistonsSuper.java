	...
	protected ArrayList<EMoveInfosExtend> listBlockExtend (World world, int x, int y, int z, int orientation, int currentOpened, int lenghtOpened) {
		...
		for (EMoveInfosExtend infos : infosExtend) {
			if (infos.block != null && infos.position != null) {
				world.setBlockTileEntity(infos.position.x, infos.position.y, infos.position.z, null);
				world.setBlock (infos.position.x, infos.position.y, infos.position.z, 0, 0, 0);
			}
		}
			...
			if (blockOrigin.block != null && blockOrigin.position != null && orientation != 0 && orientation != 1) {
				...
				if (block != null) {
					...
					if (
						...
						if (moveBlock > 0) {
							world.setBlockTileEntity(xBlock, yBlock, zBlock, null);
							world.setBlock (xBlock, yBlock, zBlock, 0, 0, 0);
							xExtension = xBlock;
							yExtension = yBlock;
							zExtension = zBlock;
