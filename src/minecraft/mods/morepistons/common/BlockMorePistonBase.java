...
public class BlockMorePistonBase extends Block {
	 ...
	 */
	public BlockMorePistonBase(int id, boolean flag, String name,
			String texturePrefixe) {

		super(id, Material.piston);

		Logger.log("Create block id : " + id + " texturePrefixe : "
				+ texturePrefixe);
	 ...
	 * @return int
	 */
	public static int determineOrientation(World par0World, int par1, int par2,
			int par3, EntityLivingBase par4EntityLivingBase) {
		if (MathHelper.abs((float) par4EntityLivingBase.posX - (float) par1) < 2.0F
				&& MathHelper.abs((float) par4EntityLivingBase.posZ
						- (float) par3) < 2.0F) {

			double d0 = par4EntityLivingBase.posY + 1.82D
					- (double) par4EntityLivingBase.yOffset;
		...
		int l = MathHelper
				.floor_double((double) (par4EntityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
	...
	public void registerIcons(IconRegister iconRegister) {
		...
		this.textureFileTopSticky = this.loadTexture(iconRegister,
				"morepistons:top_sticky");
		this.textureFileOpen = this.loadTexture(iconRegister, "morepistons:"
				+ this.texturePrefixe + "top");
		this.textureFileBottom = this.loadTexture(iconRegister, "morepistons:"
				+ this.texturePrefixe + "bottom");
		this.textureFileSide = this.loadTexture(iconRegister, "morepistons:"
				+ this.texturePrefixe + "side");
	}
	
	/**
	 * Renvoie la bonne texture en fonction de l'etat du piston
	 * @return
	 */
	...
	public Icon getIcon(int i, int metadata) {
		...
		if (i == orientation) {
			if ((isExtended(metadata)) || (this.minX > 0.0D)
		...
		return i != Facing.oppositeSide[orientation] ? this.textureFileSide
				: this.textureFileBottom;
	}
